...
library(broom)
library(tibble)
var_rep         <- vec2var(johansentest)
forecasts       <- predict(var_rep, n.ahead= 5)
dates           <- c(index(SPY)[length(SPY)] + 1, index(SPY)[length(SPY)] + 2, index(SPY)[length(SPY)] + 3,           index(SPY)[length(SPY)] + 4, index(SPY)[length(SPY)] + 5)



forecasts1 <- cbind(forecasts$fcst$SPY, forecasts$fcst$SHY) %>% 
               tidy() %>% add_column(dates)

time_series <- tidy(time_series)

    ggplot(time_series , aes(x = index, y = value, color = series)) + geom_line() +
    # Prediction intervals
   # geom_ribbon(data = forecasts1, aes(x = dates, ymin = lower, ymax = upper), 
   #             fill = "#D5DBFF", color = NA, size = 0) +
    # Actual & Forecast
    geom_line(forecasts1, aes(x = dates, y = fcst, size = 1)) + 
    #geom_point(size = 2) +
    # Aesthetics
    theme_bw() +
