...
## Stationary and Non-Stationarity Data and Issues
...
To illustrate a 'spurious regression' let's regress lagged GS prices on SPY prices. That means we are going to try to use yesterdays prices of SPY to try to predict todays GS prices.
...
The problem is that the 'statistical significance' comes from the fact that we just ran a superconsistent regression. That basically means that since the varaibles we used in the regression are (probably, we didn't prove it here) non-stationary pretty much any regression we run will conclude statistical significance in a typical t-test. In other words, the regression results below aren't valid because the statistic doesn't follow the t distribution in the first place, but when you go along as it it does follow the t distribution, you will get 'significant' results almost every time. 
```{r, results = "asis"}
library(stargazer)
GS <- GS$GS.Adjusted
...
You may be skeptical at this point. I mean, after all, it isn't that crazy to think that todays GS prices would be related to yesterday's SPY prices. In fact, it smacks of a very famous model in finance - the CAPM (we'll return to this briefly in a bit). So to illustrate, now we'll simulate two series so we are sure they are not related. Then we will regress one on the lag of the other and show statistical significance. 

For this example, I will print the code that generates the simulated prices, so you can be sure I created them independently. The simulated prices are plotted below.
...
Now we know the regression is meaningless because we simulated these prices ourselves. So that is the problem with non-stationary data in a nutshell, if you regress non-stationary variables on one another, you will always get spurious 'significant' results. 
...
Clearly, this is not exaclty white noise. We have clear periods of heightened volatility (we'll cover what to do about that in another post). But, at least the series is not trending up or down. We have solved the problem of non-stationarity by using returns instead of levels. Just to drive this point home, let's regress GS returns on lagged SPY returns and show the are no longer significant. 
...
We went through all the discussion above basically assuming that we knew whether or not the data were stationary or not. In real applied research, you always have to make a case whether or not the data are stationary to support your choice to use levels of returns in your analysis. 

There are several stationarity tests, also often called Unit Root tests. I'll list the main ones most often seen in the literature here. 
