...
# Boosting
boost.SPRet <- gbm(SPY~., data= SPRet[train], distribution = "gaussian", n.trees = 5000, interaction.depth = 4, shrinkage = 0)
summary(boost.SPRet)
...
# ETS
ets.forecast <- SPRet$SPY %>% ets %>% forecast
```
...
yhat.boost  <- predict(boost.SPRet, newdata = SPRet, n.trees = 5000)
...
indicator.ets  <- apply(as.matrix(Lag(yhat.ets$fitted)[13:length(yhat.ets$fitted)]), 1, function(x) if(x<0) (-1) else 1)
...
return.ets    <- SPRet$SPY[13:length(yhat.ets$fitted)]*indicator.ets
plot(invest*cumprod(1+return.ets))
